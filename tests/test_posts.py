...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("favorite pizza", "i love hawaii", False),
    ("tallest skyscrapers", "my house", True),
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/posts/", json={"title": "arbitrary title", "content": "asdf"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary title"
    assert created_post.content == "asdf"
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    ...
    assert res.status_code == 404
    assert res.json().get('detail') == "post with id: 8888 does not exist!"
